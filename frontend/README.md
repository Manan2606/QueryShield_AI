...
- Show bounded query results, charts, AI summaries, query history, query detail, and audit logs.
...
- Recharts for deterministic result charts
...
- `/queries/new`: protected analysis workspace with dataset selection, one Analyze action, results, chart, AI summary, and collapsed governance details.
...
AI summaries are shown only when backend-generated summary data exists. Summary generation happens after controlled execution and uses only bounded returned rows. If summaries are disabled, Gemini is unavailable, or no rows are returned, the frontend keeps the result table usable and shows the backend-provided fallback state.

Charts are inferred deterministically from returned bounded result rows when the shape is compatible with a bar, line, or pie chart. No additional SQL, BigQuery dry run, BigQuery execution, or AI call is used for visualization. Unsupported result shapes fall back to the table, which remains the source of truth.
...
The frontend deploys as a separate Cloud Run service. Set `NEXT_PUBLIC_API_BASE_URL` to the deployed backend HTTPS URL before building the container image. This value is public browser configuration, not a secret.
